...
function M.get_project_root(start_path)
  ...
  while current ~= '/' do
    if
      M.file_exists(M.path_join(current, '.xcodeproj'))
      or M.dir_exists(M.path_join(current, '*.xcodeproj'))
      or M.file_exists(M.path_join(current, '.xcworkspace'))
      or M.dir_exists(M.path_join(current, '*.xcworkspace'))
    then
      return current
    end
