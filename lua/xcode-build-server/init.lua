    ...
    ui.select_scheme(selected_project.path, function(selected_scheme)
      ui.confirm_setup(selected_project, selected_scheme, function(confirmed)
        if confirmed then
          ...
          local success =
            config.update_buildserver_config(project_dir, selected_project.path, selected_scheme)
...
function M.health()
  ...
  table.insert(results, {
    ...
    message = has_buildserver and 'Found in current directory'
      or 'Not found - run :XcodeBuildServerSetup',
  })
  ...
  table.insert(results, {
    ...
    message = project and (project.name .. ' (' .. project.type .. ')')
      or 'No Xcode project found nearby',
  })
